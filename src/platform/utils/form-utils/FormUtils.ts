...
import { isBlank, sortByField } from '../Helpers';
...
export class FormUtils {
        ...
        if (field.fields && field.fields.length) {
          for (let subfield of field.fields) {
            if (subfield.defaultValue) {
              if (isBlank(controlConfig.value)) {
                controlConfig.value = {};
              }
  ...
  toFieldSets(meta, currencyFormat, http, config: { token?: string; restUrl?: string; military?: boolean }, overrides?) {
    ...
    if (meta && meta.fields) {
      let fields = meta.fields
        ...
        .sort(sortByField(['sortOrder', 'name']));
      if (meta.sectionHeaders && meta.sectionHeaders.length) {
        meta.sectionHeaders.sort(sortByField(['sortOrder', 'name']));
  ...
  forceValidation(form: NovoFormGroup): void {
    Object.keys(form.controls).forEach((key: string) => {
      ...
      if (control.required && isBlank(form.value[control.key])) {
        control.markAsDirty();
